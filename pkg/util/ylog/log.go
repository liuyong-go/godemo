...
func InitLog() {
	...
	encoderConfig := zapcore.EncoderConfig{
		MessageKey:   conf.Conf.YLog.EncodeConfig.MessageKey,
		LevelKey:     conf.Conf.YLog.EncodeConfig.LevelKey,  //结构化（json）输出：日志级别的key（INFO，WARN，ERROR等）
		TimeKey:      conf.Conf.YLog.EncodeConfig.TimeKey,   //结构化（json）输出：时间的key（INFO，WARN，ERROR等）
		CallerKey:    conf.Conf.YLog.EncodeConfig.CallerKey, //结构化（json）输出：打印日志的文件对应的Key
	...
	infoWriter := getWriter(conf.Conf.YLog.InfoPath)
	warnWriter := getWriter(conf.Conf.YLog.ErrorPath)
	var core zapcore.Core
	if conf.Conf.YLog.Development == true {
		core = zapcore.NewTee(
...
func getWriter(filename string) io.Writer {
	hook, err := rotatelogs.New(
		...
		rotatelogs.WithMaxAge(time.Hour*conf.Conf.YLog.RotationLogs.MaxAge),            // 保存30天
